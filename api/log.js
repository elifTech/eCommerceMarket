...
var bunyan = require('bunyan'),
  ...
  logRecords = require('./dependencies/logRecord');
...
var writer = {
  write: function (obj) {
    ...
    logRecords.create(logRecord, function (err) {
...
if (config.get('log.syslog.enabled')) {
  try {
    if (require.resolve('node-syslog')) {
      ...
      Syslog.init('example.com', Syslog.LOG_PID | Syslog.LOG_ODELAY, Syslog.LOG_LOCAL0);
      var writer = {
        write: function (obj) {
          var level;
